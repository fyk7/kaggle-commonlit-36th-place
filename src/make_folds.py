...
def mk_fold_by_stratified_url(
    ...
    validationにはStratifiedKfoldを採用
    """
...
def mk_fold_by_difficulty_bins(
    ...
    train_df['Fold'] = -1
    bin_kfold = StratifiedKFold(
        n_splits=cfg.nfolds,
        shuffle=True,
        random_state=cfg.seed
    )
    for k , (train_idx, valid_idx) in \
        enumerate(bin_kfold.split(X=train_df, y=bins)):
        train_df.loc[valid_idx, 'Fold'] = k

    return train_df[["id", "Fold"]]

def mk_fold_by_stratified_bin_group_url(
    train_df: pd.DataFrame,
    cfg: DictConfig,
    fold_name: str = "Fold_stratified_bin_group_url",
) -> pd.DataFrame:
    """
    文章の読みやすさ(readability)をbinに分割してから、
    そのbinに対してStratifiedKFoldする
    さらにurlが被ってリークしないように、urlに対してGroupKfoldする
    """
    num_bins = int(np.floor(1 + np.log2(len(train_df))))
    train_df['bins'] = pd.cut(
        train_df['target'], bins=num_bins, labels=False)
    ...
    for k , (train_idx, valid_idx) in \
        enumerate(stratified_group_kfold.split(
            X=train_df, y=bins, groups=train_df["url_base_int"])):
        train_df.loc[valid_idx, 'Fold'] = k
...
def main(cfg: DictConfig):
    ...
    # train_df_with_fold = mk_fold_by_stratified_bin_group_url(train_df, cfg, fold_name)
